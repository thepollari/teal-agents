...
func (kb *KernelBuilder) BuildKernel(ctx context.Context, agentConfig types.AgentConfig) (types.Kernel, error) {
	...
	if len(agentConfig.Plugins) > 0 {
		logger.Warn("Local plugins not supported in distributed binary", "plugins", agentConfig.Plugins)
		logger.Info("Use remote_plugins instead for distributed deployment")
	}
...
func (kb *KernelBuilder) AddPlugins(ctx context.Context, kernel types.Kernel, pluginNames []string) error {
	logger := logging.WithContext(ctx)
	logger.Warn("Local plugin loading not supported", "plugins", pluginNames)
	logger.Info("Use remote plugins via plugin catalog instead")
	return fmt.Errorf("local plugins not supported in distributed binary - use remote plugins")
}
