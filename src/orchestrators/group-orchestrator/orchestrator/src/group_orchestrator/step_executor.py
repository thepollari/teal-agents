...
class StepExecutor:
    ...
    async def _execute_task(self, task: ExecutableTask) -> None:
        ...
        task_result = await task_agent.perform_task(task.task_goal, pre_requisites)
        task.result = task_result
        task.status = TaskStatus.DONE
        self.task_accumulator[task.task_id] = task

    async def execute_step(self, step: Step) -> None:
        await asyncio.gather(*[self._execute_task(task) for task in step.step_tasks])
