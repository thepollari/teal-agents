...
class AioBaseAgentBuilder:
    def __init__(self, gateway: AioAgentGateway):
    ...
    def _get_agent_description(self, agent_name: str) -> str:
        response = requests.get(
            f"{self._http_or_https()}://{self.gateway.host}/{AioBaseAgentBuilder._agent_to_path(agent_name)}/openapi.json"
        )
        if response:
