...
@patch("sk_agents.authorization.authorizer_factory.ModuleLoader.load_module")
def test_class_not_subclass_of_request_authorizer_raises_type_error(mock_load_module, mock_app_config):  # noqa: E501
...
def test_missing_module_env_variable_raises_value_error(mock_app_config):
    mock_app_config.get.side_effect = lambda key: None if key == TA_AUTHORIZER_MODULE.env_name else "SomeClass"  # noqa: E501
...
def test_missing_class_env_variable_raises_value_error(mock_app_config):
    mock_app_config.get.side_effect = lambda key: "some.module" if key == TA_AUTHORIZER_MODULE.env_name else None  # noqa: E501
