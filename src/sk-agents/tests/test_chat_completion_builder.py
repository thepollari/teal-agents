...
def test_init_with_no_module(app_config_fixture, setup_env_fixture):
...
def test_init_with_custom_module(app_config_fixture, setup_env_fixture):
    chat_completion_builder = ChatCompletionBuilder(app_config_fixture)
    assert isinstance(chat_completion_builder, ChatCompletionBuilder)


def test_init_custom_chat_completion_class_not_found(app_config_fixture, setup_env_fixture):
...
def test_init_custom_chat_completion_invalid_class(app_config_fixture, setup_env_fixture):
...
def test_get_chat_completion_for_model_error(app_config_fixture, setup_env_fixture):
    chat_completion_builder = ChatCompletionBuilder(app_config_fixture)
    with pytest.raises(ValueError):
        chat_completion_builder.get_chat_completion_for_model("test-id", "unsuppored-model")


def test_get_model_type_for_name(app_config_fixture, setup_env_fixture):
    chat_completion_builder = ChatCompletionBuilder(app_config_fixture)
    model_type = chat_completion_builder.get_model_type_for_name("gpt-model")
    assert model_type == "openai"


def test_get_model_type_for_name_error(app_config_fixture, setup_env_fixture):
    chat_completion_builder = ChatCompletionBuilder(app_config_fixture)
    with pytest.raises(ValueError):
        chat_completion_builder.get_model_type_for_name("unsupported-model")


def test_get_model_supports_structured_output(app_config_fixture, setup_env_fixture):
    chat_completion_builder = ChatCompletionBuilder(app_config_fixture)
    does_support = chat_completion_builder.model_supports_structured_output(model_name="gpt-model")
    assert does_support is True


def test_get_model_supports_structured_output_error(app_config_fixture, setup_env_fixture):
    chat_completion_builder = ChatCompletionBuilder(app_config_fixture)
    with pytest.raises(ValueError):
