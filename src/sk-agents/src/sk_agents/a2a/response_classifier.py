...
class A2AResponseClassifier:
    ...
    SYSTEM_PROMPT = (
        ...
        '(referred to as the "Primary Agent") and classifying its status. Your output MUST be a JSON '
        "object.**\n"
        "\n"
        ...
        "        }\n"
        "        ```\n"
        '    * Keywords/phrases to look for: "done," "completed," "finished," "success," "here '
        '        is the result," "I have finished," "the task is complete," direct answers to '
        ...
        "        }\n"
        "        ```\n"
        '    * Keywords/phrases to look for: "failed," "unable to," "cannot complete," "error," '
        '        "encountered a problem," "not possible," "I\'m sorry, I can\'t," "task aborted."\n'
        ...
        '          "message": "A description of what information is needed from the user and why."\n'
        "        }\n"
        "        ```\n"
        ...
        '          "auth_details": {} // Likely a JSON structure extracted from the Primary Agent\'s '
        "output containing technical details about the auth request. Can be an empty object if "
        "no specific structure is found.\n"
        "        }\n"
        "        ```\n"
        '    * Keywords/phrases to look for: "please log in," "authentication required," "access '
        '        denied," "invalid credentials," "API key needed," "sign in to continue," "verify '
        '        your identity," "permissions needed."\n'
        "    * Context: The output explicitly states or strongly implies that a security or access "
        "        barrier is preventing task progression. The `message` field should explain this. "
        "        The `auth_details` field should attempt to capture any structured information "
        ...
        "\n"
        "1.  **Carefully review the entire output from the Primary Agent.** Understand the context "
        ...
        "4.  **Prioritize:**\n"
        "    * If authentication is mentioned as a blocker, classify as `auth-required`. Extract "
        ...
        "    * If the agent explicitly states failure or an insurmountable error (not related to "
        ...
        "    * For `auth-required`, if the Primary Agent's output includes a structured (e.g., "
        "        JSON) segment detailing the authentication requirements, attempt to extract and "
        "        place this into the `auth_details` field. If no specific structure is found, "
        ...
        "**Example Scenarios:**\n"
        "\n"
        "* **Primary Agent Output:** \"I've finished generating the report you asked for. It's "
        '    attached below."\n'
        "    * **Your JSON Output:**\n"
        "        ```json\n"
        "        {\n"
        ...
        "        {\n"
        '          "status": "input-required",\n'
        '          "message": "The agent needs to know the specific date range for the sales data '
        'to proceed."\n'
        "        }\n"
        "        ```\n"
        '* **Primary Agent Output:** "Access to this API endpoint requires authentication. Please '
        ...
        "        }\n"
        "        ```\n"
        '* **Primary Agent Output:** "You need to sign in to your account to access your profile. '
        '    Click here to login."\n'
        "    * **Your JSON Output:**\n"
        "        ```json\n"
        "        {\n"
